...
| Wolf death camera shake | 2 | Reusable local positional shake and a Wolf-death camera sequence are implemented; wire/tune the active camera and authored focus targets. Camera remains local-only in multiplayer. |
| Falling-tree sequence | 2 | Four scene-tree responders work; the cinematic now owns their major trigger timing while per-tree delays provide only stagger. Final timing/framing needs Play Mode tuning. |
| Reveal/open route after Wolf death | 2 | The cinematic can reveal route objects and release deferred encounter collision at the tree-opening beat; exact exit references and visual validation remain. |
...
| Recruit 3 allies | 4 | Three data-driven villagers transition Ambient → RecruitReady → Following, can warn when the Player persistently leaves an authored raid-route corridor, abandon independently, navigate home, and be recruited again. Persistence and party UI remain; recruitment state must eventually be authoritative/synchronized. |
| Ally AI | 4 | Recruited allies follow in formation with NavMesh-driven locomotion and can be handed to a bidirectional, explicitly authored stepping-stone jump sequence one at a time. Scene authoring/QA, hostile target selection, combat, and recovery remain; AI decisions/state will need synchronization. |
