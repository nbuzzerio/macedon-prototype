...
| Recruit 3 allies | 4 | Three data-driven villagers now transition Ambient → RecruitReady → Following through the existing interaction, register idempotently into distinct formation slots, and follow via guarded NavMesh movement. Route warnings/abandonment/return/re-recruit, persistence, and party feedback remain; recruitment state must eventually be authoritative/synchronized. |
| Ally AI | 4 | Recruited allies follow in formation and now have NavMesh-velocity-driven idle/walk/run presentation. Jump/traversal choreography, hostile target selection, combat, and recovery remain; AI decisions/state will need synchronization. |
